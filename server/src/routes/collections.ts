...
import { CollectionRecord, Storage } from '../storage';
...
/**
 * Collections (saved Scenes) resolve through the same injected storage as the shows and production
 * routers, so a Show and its Scenes are served by one storage (previously this router read a module
 * singleton, which diverged from the injected dataStorage in tests).
 */
export function createCollectionsRouter(storage: Storage): Router {
  const router = Router();
  ...
  /**
   * POST /api/shows/:showId/scenes — create a new independent Scene bound to the Show (AC-006).
   * A Show-scoped, first-class Scene creation: the Scene's channel is the Show and it starts at
   * revision 1. Body: { name, scene?, variables? }.
   */
  router.post('/shows/:showId/scenes', async (req: Request, res: Response) => {
    const tenantId = resolveTenant(req);
    const showId = req.params.showId;
    const show = await storage.getShow(showId);
    if (!show || show.archivedAt !== null) {
      return res.status(404).json({ error: { code: 'SHOW_NOT_FOUND', message: 'Show not found' } });
    }
    const { name, scene, variables } = req.body || {};
    ...
    logger.info('Scene created in Show', { tenantId, showId, id: saved.id });
    res.status(201).json({
      data: { id: saved.id, name: saved.name, channelId: saved.channelId, revision: saved.revision },
    });
  });
